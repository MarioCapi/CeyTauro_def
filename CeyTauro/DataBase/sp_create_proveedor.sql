...
CREATE OR REPLACE PROCEDURE "Management".sp_update_proveedor (IN _nit_proveedor BIGINT, IN _nombre_contacto VARCHAR(255), IN _razon_social VARCHAR(255), IN _telefono_contacto VARCHAR(50), IN _direccion VARCHAR(255), IN _correo_electronico VARCHAR(255)) LANGUAGE plpgsql AS $$
BEGIN
...
CREATE OR REPLACE PROCEDURE "Management".sp_delete_proveedor( IN p_nit_proveedor VARCHAR, OUT p_resultado JSON) LANGUAGE plpgsql AS $$
BEGIN
